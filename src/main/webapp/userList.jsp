...
<table class="list">
    <th colspan="6">
        <form action="${pageContext.request.contextPath}/users/findByLike" method="post">
            <input type="text" placeholder="按用户名搜索" name="userName"
                   id="search-input" class="search-input"/> <input type="submit" id="search-button" value="搜索"></input>
        </form>
    </th>
    <tr class="title-tr">
        <td>Id</td>
    ...
    %>
    <tr class="value-tr">
        <td class="value-td"><%=u.getId()%>
        </td>
        <td class="value-td"><%=u.getUserName()%>
        </td>
        <td class="value-td pswd"><%=u.getUserPassword()%>
        </td>
        <td class="value-td"><%=u.getPhone()%>
        </td>
        <td class="value-td"><%=u.getEmail()%>
        </td>
